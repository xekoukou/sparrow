...
mutual
  ...
  compl-repl⇒id-abs1 ind eq eq1 eq2 (yes refl) = {!!}
  compl-repl⇒id-abs1 {d = ic←} {w1} ind eq eq1 eq2 (no refl) = {!!}
  compl-repl⇒id-abs1 {d = ic→} {w1} ind eq eq1 eq2 (no refl) = {!!} -- trans (mapₛ-fg {g = s-extend ind} {f = sic d} w1) {!!}
...
oi&ss≤s⇒oiss .↓ ss .↓ {{oIs↓}} {{⊂↓}} = oIs↓
oi&ss≤s⇒oiss .(sic _ _) .(sic _ _) .↓ {{oIs↓}} {{⊂sic}} = oIs↓
oi&ss≤s⇒oiss (sic _ s) (sic _ ss) (ic _ ind) {{oIic}} {{⊂sic}} = oIic {{ieq = oi&ss≤s⇒oiss s ss ind}}
oi&ss≤s⇒oiss .(sbc _ _) .(sbc _ _) .↓ {{oIs↓}} {{⊂sbc}} = oIs↓
oi&ss≤s⇒oiss .(sbc _ _) .(sic _ _) .↓ {{oIs↓}} {{⊂dsbc}} = oIs↓



ho&s≤ss⇒hoss : ∀ {i u ll pll} → (s ss : SetLL ll) → (ind : IndexLL {i} {u} pll ll)
               → {{ho : hitsAtLeastOnce s ind}} → {{eq : s ⊂ₛ ss}} → hitsAtLeastOnce ss ind
ho&s≤ss⇒hoss .↓ .↓ .(ic _ _) {{hLO↓ic}} {{⊂↓}} = hLO↓ic
ho&s≤ss⇒hoss s .↓ .↓ {{hLOs↓}} {{⊂↓}} = hLOs↓
ho&s≤ss⇒hoss .(sic _ _) .↓ .(ic _ _) {{hLOsic}} {{⊂↓}} = hLO↓ic
ho&s≤ss⇒hoss .(sbc _ _) .↓ .(ic _ _) {{hLOsbc}} {{⊂↓}} = hLO↓ic
ho&s≤ss⇒hoss .(sic _ _) .(sic _ _) .↓ {{hLOs↓}} {{⊂sic}} = hLOs↓
ho&s≤ss⇒hoss (sic _ s) (sic _ ss) (ic _ ind) {{hLOsic}} {{⊂sic}} = hLOsic {{ieq = ho&s≤ss⇒hoss s ss ind}}
ho&s≤ss⇒hoss (sbc _ s) (sbc _ ss) .↓ {{hLOs↓}} {{⊂sbc}} = hLOs↓
ho&s≤ss⇒hoss (sbc s s1) (sbc ss ss1) (ic d ind) {{hLOsbc}} {{⊂sbc}} = hLOsbc {{ieq = ho&s≤ss⇒hoss (pickLLₛ d s s1) (pickLLₛ d ss ss1) ind {{eq = ⊂ₛ-pickLLₛ d}}}}
ho&s≤ss⇒hoss (sic _ s) (sbc _ ss) .↓ {{hLOs↓}} {{⊂dsbc}} = hLOs↓
ho&s≤ss⇒hoss (sic _ s) (sbc ss ss1) (ic d ind) {{hLOsic}} {{⊂dsbc}} = hLOsbc {{ieq = ho&s≤ss⇒hoss s (pickLLₛ d ss ss1) ind}}



¬ho&s≤ss⇒¬hos : ∀ {i u ll pll} → (s ss : SetLL ll) → (ind : IndexLL {i} {u} pll ll)
                → ¬ (hitsAtLeastOnce ss ind) → {{eq : s ⊂ₛ ss}} → ¬ (hitsAtLeastOnce s ind)
¬ho&s≤ss⇒¬hos s ss ind ¬ho x = x asInst ¬ho (ho&s≤ss⇒hoss s ss ind)
