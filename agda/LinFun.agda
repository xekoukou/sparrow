...
-- calls in LinLogic need to be excluded from this, thus we need to add-calls in SetLL.
  ...
  data usesInputT` : {i : Size} → {j : Size< ↑ i } → ∀{u rll ll} → LFun {u} {i} {j} {rll} {ll} → MSetLL ll → Set where
    usesInputC`⊂↓∅ : {i : Size} → {j : Size< ↑ i} → {k : Size< ↑ j} → ∀{u rll ell ll elf prf lf}
                    → usesInputT` {i} {k} (_⊂_ {u} {i} {j} {k} {ll} {ll} {ell} {rll} {↓} elf {{prf}} lf) ∅
    usesInputC`⊂↓¬∅c : {i : Size} → {j : Size< ↑ i} → {k : Size< ↑ j} → ∀{u rll ell ll s elf prf lf}
                     → {cTo↓ : (contruct $ add s ↓ ell) ≡ ↓} 
                     → usesInputT` {i} {k} (_⊂_ {u} {i} {j} {k} {ll} {ll} {ell} {rll} {↓} elf {{prf}} lf) (¬∅ s)
    usesInputC`⊂↓¬∅¬c : {i : Size} → {j : Size< ↑ i} → {k : Size< ↑ j} → ∀{u rll ell ll s elf prf lf}
                      → let ns = contruct $ add s ↓ ell in {¬cTo↓ : ¬ (ns ≡ ↓)}
                      → usesInputT` {j} {k} {u} {rll} {replLL ll ↓ ell} lf (¬∅ ns)
                      → usesInputT` {i} {k} (_⊂_ {u} {i} {j} {k} {ll} {ll} {ell} {rll} {↓} elf {{prf}} lf) (¬∅ s)
...
doesItUseAllInputs {ll = ll} lf with (doesItUseAllInputs` lf ∅) where
  ...
  doesItUseAllInputs` (_⊂_ {ell = ell} {ind = ↓} elf lf) ∅ = yes usesInputC`⊂↓∅
  doesItUseAllInputs` plf@(_⊂_ {ell = ell} {ind = ↓} elf lf) (¬∅ s) with (isEq (contruct $ add s ↓ ell) ↓)
  ... | yes cTo↓ = yes (usesInputC`⊂↓¬∅c {cTo↓ = cTo↓})
  ... | no ¬cTo↓ with (doesItUseAllInputs` lf (¬∅ $ contruct $ add s ↓ ell))
...
findUnusedorPrUI : {i : Size} → {j : Size< ↑ i } → ∀{u rll ll} → (lf : LFun {u} {i} {j} {rll} {ll}) → (usesInputT lf) ⊎ (SetLLRem ll rll)
findUnusedorPrUI {ll = ll} lf with (findUnusedorPrUI` {pll = ll} lf ∅ ↓) where
  findUnusedorPrUI` : {pi : Size} → {i : Size< ↑ pi} → {j : Size< ↑ i } → ∀{u rll pll ll} → (lf : LFun {u} {i} {j} {rll} {ll}) → (ms : MSetLL ll) → (sr : SetLLRem {pi} pll ll) → (usesInputT` lf ms) ⊎ (SetLLRem pll rll)
  findUnusedorPrUI` I ms sr = inj₂ sr
  findUnusedorPrUI` (_⊂_ {ell = ell} {ind = ↓} elf lf) ∅ sr = inj₁ usesInputC`⊂↓∅
  findUnusedorPrUI` plf@(_⊂_ {ell = ell} {ind = ↓} elf lf) (¬∅ s) with (isEq (contruct $ add s ↓ ell) ↓)
  ... | yes cTo↓ = yes (usesInputC`⊂↓¬∅c {cTo↓ = cTo↓})
  ... | no ¬cTo↓ with (findUnusedorPrUI` lf (¬∅ $ contruct $ add s ↓ ell))
  ... | yes p = yes (usesInputC`⊂↓¬∅¬c {¬cTo↓ = ¬cTo↓} p)
  ... | no p  = no hf where
    hf : usesInputT` plf (¬∅ s) → ⊥
    hf (usesInputC`⊂↓¬∅c {cTo↓ = cTo↓}) = ¬cTo↓ cTo↓
    hf (usesInputC`⊂↓¬∅¬c x) = p x
  findUnusedorPrUI` plf@(_⊂_ {ell = ell} {ind = ind ←∧} elf lf) ∅ with (findUnusedorPrUI` lf (¬∅ $ ∅-add (ind ←∧) ell))
  ... | yes p = yes (usesInputC`⊂←∧∅ p)
  ... | no ¬p = no hf where
    hf : usesInputT` plf ∅ → ⊥
    hf (usesInputC`⊂←∧∅ x) = ¬p x
  findUnusedorPrUI` plf@(_⊂_ {ell = ell} {ind = (ind ←∧)} elf lf) (¬∅ s) with (isEq (contruct $ add s (ind ←∧) ell) ↓)
  ... | yes cTo↓ = yes (usesInputC`⊂←∧¬∅c {cTo↓ = cTo↓})
  ... | no ¬cTo↓ with (findUnusedorPrUI` lf (¬∅ $ contruct $ add s (ind ←∧) ell))
  ... | yes p = yes (usesInputC`⊂←∧¬∅¬c {¬cTo↓ = ¬cTo↓} p)
  ... | no p  = no hf where
    hf : usesInputT` plf (¬∅ s) → ⊥
    hf (usesInputC`⊂←∧¬∅c {cTo↓ = cTo↓}) = ¬cTo↓ cTo↓
  ...
  ... | no p  = no hf where
    ...
    hf (usesInputC`⊂∂→¬∅c {cTo↓ = cTo↓}) = ¬cTo↓ cTo↓
    hf (usesInputC`⊂∂→¬∅¬c x) = p x
  findUnusedorPrUI` plf@(tr lf) ∅ with (findUnusedorPrUI` lf ∅)
  ... | yes p = yes (usesInputC`tr∅ p)
  ... | no ¬p = no hf where
    hf : usesInputT` plf ∅ → ⊥
    hf (usesInputC`tr∅ x) = ¬p x
  findUnusedorPrUI` plf@(tr {{ltr = ltr}} lf ) (¬∅ s) with (findUnusedorPrUI` lf $ ¬∅ (SetLL.tran s ltr))
  ... | yes p  = yes (usesInputC`trs p)
  ... | no ¬p  = no hf where
    hf : usesInputT` plf (¬∅ s) → ⊥
    hf (usesInputC`trs x) = ¬p x
-- Since we should never reach at obs, we set it to ⊥ in hopes of catching any bugs.
  findUnusedorPrUI` (obs lf) ms = no (λ ())
  findUnusedorPrUI` (com df lf) ms = yes usesInputC`com
-- We add all the calls that are here and check if together with ms it contructs to ↓.
  findUnusedorPrUI` {ll = ll} plf@(call x lf) ms with (isEqM ((fillWithCalls ll) c+ₘₛ ms) (¬∅ ↓) )
  ... | yes cTo↓ = yes (usesInputC`callc {cTo↓ = cTo↓})
  ... | no ¬cTo↓ = no hf where
    hf : usesInputT` plf ms → ⊥
    hf (usesInputC`callc {cTo↓ = cTo↓}) = ¬cTo↓ cTo↓
